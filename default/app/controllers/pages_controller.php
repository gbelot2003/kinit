...
class PagesController extends AppController 
{
	...
	{
		$this->menu['contact'] = "active";
		
		if(Input::hasPost("titulo", "nombre", "email", "cuerpo")){
			$this->titulo = Input::post('titulo');
			$this->nombre = Input::post('nombre');
			$this->email = Input::post('email');
			$this->cuerpo = Input::post('cuerpo');
			Mail::sendContactMail($this->nombre, $this->email, $this->titulo, $this->cuerpo);
		}
	}
}
